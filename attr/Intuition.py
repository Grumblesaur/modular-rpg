...
def s_disguise(intuition, proficiency, dc):
  '''(2 GP) Transform into other people, if you have the materials.'''
  return skill_check(intuition, proficiency, dc)

def s_forage(intuition, proficiency, dc):
  '''(2 GP) Scavenge the environment for organic materials.'''
  return skill_check(intuition, proficiency, dc)

def s_listen(intuition, proficiency, dc):
  '''(2 GP) Tune in to nearby sounds.'''
  return skill_check(intuition, proficiency, dc)

def s_look(intuition, proficiency, dc):
  '''(2 GP) Observe your environment.'''
  return skill_check(intuition, proficiency, dc)

def s_search(intuition, proficiency, dc):
  '''(2 GP) Scour the environment for lost items or hidden objects.'''
  return skill_check(intuition, proficiency, dc)

def s_sense_motive(intuition, proficiency, dc):
  '''(2 GP) Get a feel for another's intentions, and the veracity of their
  statements.'''
  return skill_check(intuition, proficiency, dc)
